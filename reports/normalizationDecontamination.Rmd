...
# load data
load("~/git/amr-brazil/R/genecountsENV.RData")
...
seqs <- read.table("/Users/epb5360/git/amr-brazil/data/sampleDepth.txt", stringsAsFactors = FALSE, header = TRUE, sep = "\t") %>% 
  rename(names = name,
         nreads = length)
...
Finally, we can use the metadata encoded in gene ID and sample name to get important grouping variables for downstream analysis. 
...
write.table(relv, file = "~/git/amr-brazil/data/parsedRelativeAbundanceNoContams.txt", sep = "\t",
            row.names = FALSE)
