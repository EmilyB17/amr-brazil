...
counts <- read.table("https://github.com/EmilyB17/amr-brazil/blob/master/data/parsedRelativeAbundanceNoContams.txt?raw=true", sep = "\t",
                    header = TRUE)
...
## use presence/absence to remove genes only present in 1 sample
...
The data is so zero-skewed that it can be misleading to plot just the median and IQR, so below is a plot of whole dataset in the significant genes.
...
sum <- sigs %>% 
  ...
  select(-c(pval, sig)) %>% 
  pivot_wider(names_from = site, values_from = c(median, IQR, prestotal)) %>% 
  select(-pattern) %>% 
  arrange(farm)
